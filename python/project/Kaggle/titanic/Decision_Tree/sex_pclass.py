...
y_train_pred = dt.predict(sex_pclass)
df_train_pred = pd.concat([sex_pclass,pd.DataFrame(y_train_pred)],axis=1) 
y_pred = dt.predict(test_sex_pclass)
y_pred_df = pd.DataFrame(y_pred)
y_pred_df.set_index(test.PassengerId,inplace=True)
y_pred_df.columns = ['Survived']
y_pred_df.to_csv("dt_sex_pclass.csv")
